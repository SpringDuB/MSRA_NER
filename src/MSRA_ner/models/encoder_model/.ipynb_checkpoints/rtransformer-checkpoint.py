...
from src.MSRA_ner.models.encoder_model._base import EncoderModel
...
class LocalRNN(nn.Module):
    def __init__(self, input_dim, output_dim, rnn_type, ksize, dropout):
        ...
        self.zeros = torch.zeros((self.ksize - 1, input_dim)).to(device)  # [窗口大小-1，dim]
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            x = layer(x)  # local rnn的执行
        x = self.connections[0](x, self.pooling)  # 多头attention的执行
        x = self.connections[1](x, self.feed_forward)  # FFN全连接的执行
        return x
...
class RTransformerEncoderModel(EncoderModel):
    def __init__(self, input_dim: int, param: Param, *args, **kwargs):
        super(RTransformerEncoderModel, self).__init__(
            input_dim=input_dim,
