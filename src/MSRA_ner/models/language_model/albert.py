...
from transformers import AlbertModel, AlbertTokenizer  # pip install transformers
...
class ALBertLanguageModel(LanguageModel):
    tokenizer_class = AlbertTokenizer

    def __init__(self, param: Param):
        super(ALBertLanguageModel, self).__init__(
            output_dim=param.albert_config.hidden_size,
            vocab_size=param.lm_args['vocab_size']
        )

        _args = param.lm_args
        _cfg = param.albert_config
        try:
            # 迁移模型
            pretrained_model_name_or_path = _args.get('fine_tune_root_dir')
            if pretrained_model_name_or_path is None:
                self.bert = AlbertModel(config=_cfg, add_pooling_layer=False)
            else:
                self.bert = AlbertModel.from_pretrained(
                    config=_cfg,
                    pretrained_model_name_or_path=pretrained_model_name_or_path
                )
                print("Albert模型参数迁移成功!")
        except Exception as e:
            print(f"迁移Albert模型异常:{e}")
            self.bert = AlbertModel(config=_cfg, add_pooling_layer=False)
