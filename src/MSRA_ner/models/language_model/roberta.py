...
from transformers import RobertaModel, RobertaTokenizer  # pip install transformers
...
class RobertaLanguageModel(LanguageModel):
    ...
    def __init__(self, param: Param):
        super(RobertaLanguageModel, self).__init__(
            output_dim=param.roberta_config.hidden_size,
            vocab_size=param.lm_args['vocab_size']
        )

        _args = param.lm_args
        _cfg = param.roberta_config
        try:
            # 迁移模型
            pretrained_model_name_or_path = _args.get('fine_tune_root_dir')
            if pretrained_model_name_or_path is None:
                self.bert = RobertaModel(config=_cfg, add_pooling_layer=False)
            else:
                self.bert = RobertaModel.from_pretrained(
                    config=_cfg,
                    pretrained_model_name_or_path=pretrained_model_name_or_path
                )
                print("Roberta模型参数迁移成功!")
        except Exception as e:
            print(f"迁移Roberta模型异常:{e}")
            self.bert = RobertaModel(config=_cfg, add_pooling_layer=False)
