...
from src.MSRA_ner.utils.constants import NER_ENTITY_START_ID
import time

# noinspection PyUnresolvedReferences
@torch.no_grad()
def accuracy(pred_label_ids, label_ids, input_mask) -> (float, float, float):
    """
    ...
    :return:
    """
    cor = (pred_label_ids == label_ids).to(label_ids.dtype)  # [N,T]
    cor = cor * input_mask  # 将填充位置的值设为0
    numerator = cor.sum().item()  # 得到有多少个是预测正确的
    denominator = input_mask.sum().item()
    acc = 0.0 if denominator <= 0.0 else numerator / denominator
    return acc, numerator, denominator


# noinspection PyUnresolvedReferences
@torch.no_grad()
def ner_accuracy(pred_label_ids, label_ids) -> (float, float, float):
    """
    ...
    :return:
    """
    # print(pred_label_ids, label_ids)
    # time.sleep(1)
    cor = (pred_label_ids == label_ids).to(label_ids.dtype)  # [N,T]
    mask = (label_ids >= NER_ENTITY_START_ID).to(label_ids.dtype)  # [N,T]
    cor = cor * mask  # 将所有特殊标签位置的值设为0
    numerator = cor.sum().item()  # 得到有多少个是预测正确的
